...
export const authOptions: AuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user, account, profile }) {
      // This runs on signIn
      if (user) {
        // If OAuth login, make sure we store MongoDB _id
        if (account?.provider === "google" || account?.provider === "github") {
          ...
          if (existingUser) {
            token.user = {
              id: existingUser._id.toString(), // ✅ Always MongoDB _id
        ...
        // If credentials login, user object is already populated with Mongo _id
